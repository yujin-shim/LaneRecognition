...
template <typename PREC>
std::tuple<Indices, Indices, float> HoughTransformLaneDetector<PREC>::divideLines(const Lines& lines)
{
    ...
    return { leftLineIndices, rightLineIndices, slope };
}
...
std::pair<int32_t, int32_t> HoughTransformLaneDetector<PREC>::getLanePosition(const cv::Mat& image)
{
    ...
    const auto [leftLineIndices, rightLineIndices, slope] = divideLines(allLines);
    ...
    if (!(leftPositionX == 0 && rightPositionX == 0 )){

        if (leftPositionX == 0)
        {
            //double degree = slope * (180/M_PI);
            //leftPositionX = rightPositionX - 350/sin(degree);
            //rightPositionX = rightPositionX + 20;
            leftPositionX = rightPositionX - 400;
            rightPositionX = rightPositionX + 50;
        ...
        {
            //double degree = slope * (180/M_PI);
            //rightPositionX = leftPositionX + 350/sin(degree);
            leftPositionX = leftPositionX - 250;
            rightPositionX = leftPositionX + 400;
